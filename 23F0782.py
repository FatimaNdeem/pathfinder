...
# Panel height (fixed)
PANEL_HEIGHT = 200  # Increased to accommodate metrics
...
class PathfindingApp:
    ...
    def __init__(self,rows,cols):
        ...
        # Window dimensions (grid + panel BELOW)
        self.window_width = max(self.grid_width, 700)  # Minimum width for metrics
    ...
    def layout_rect(self,r,c):
        # Center grid horizontally if window is wider
    ...
    def recalculate_path(self):
        ...
        self.metrics['path_cost'] = len(path) - 1 if path else 0  # Number of steps (edges)
        self.metrics['execution_time'] = (end_time - start_time) * 1000  # Convert to milliseconds
        self.metrics['last_execution_time'] = time.strftime('%H:%M:%S')  # Timestamp
